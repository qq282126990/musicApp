<template>
    <div>
        ...
        <!--歌曲列表-->
        <song-list-play-all :totalSongNum="getFavoriteList.length"></song-list-play-all>
        <scroll
            class="song-list-wrapper" ref="scroll">
            <song-list
                :totalSongNum="getFavoriteList.length"
...
<script type="text/ecmascript-6">
    ...
    export default{
        ...
        mounted () {
            // 滚动组件传入的数据
            this.setScrollData(this.getFavoriteList);

            // 设置歌曲列表
            this.setSongList(this.getFavoriteList);
        },
        // 组件激活
        activated() {
            // 设置歌曲列表
